...
def get_stay_details(id):
    try:
        conn = get_db_connection() # Your function to get database connection
        cursor = conn.cursor(dictionary=True)
        ...
        cursor.execute('''
            SELECT
                s.*,
                u.name as host_name,
                u.rating as host_rating,  # Assuming you want host rating
                u.reviews as host_reviews, # Assuming you want host reviews
                GROUP_CONCAT(
                    DISTINCT CONCAT(si.image_path, ':', COALESCE(si.display_order, 0))
                    ORDER BY si.display_order ASC
                ) as image_data,
        ...
        if stay:
            # Process stay data (similar to your update_stay, but for GET)
            stay['price_per_night'] = float(stay['price_per_night'])
            stay['created_at'] = stay['created_at'].isoformat()
            stay['updated_at'] = stay['updated_at'].isoformat()
            ...
            return jsonify({'stay': stay}), 200
        else:
            return jsonify({'message': 'Stay not found'}), 404

    except Exception as e:
        print("Error fetching stay details:", str(e))
        return jsonify({'message': 'Failed to fetch stay details', 'error': str(e)}), 500
    finally:
        if 'cursor' in locals():
            cursor.close()
        if 'conn' in locals():
